...
func body_enter(body):
	if(body.get_name() == "PlayerBody"):
		...
		get_tree().get_root().get_node("Game/UI/InfoUI/Panel/Label").set_text("Press 'e' to stop timer")
		get_tree().get_root().get_node("Game").isNearEndButton = true

func body_exit(body):
	get_tree().get_root().get_node("Game/UI/InfoUI/Panel").hide()
	get_tree().get_root().get_node("Game").isNearEndButton = false
